...
function initAudio() {
  ...
  audioReady = true;
  if (typeof window._gameSfxVolume === "number") setGameSfxVolume(window._gameSfxVolume);
}

function setGameSfxVolume(percent) {
  const clamped = Math.max(0, Math.min(100, Number(percent) || 0));
  window._gameSfxVolume = clamped;
  if (masterGainNode) masterGainNode.gain.value = 0.22 * (clamped / 100);
}
...
function playLaser() {
  ...
  filt.Q.value = 3;
  osc2.connect(filt); filt.connect(g2); g2.connect(masterGainNode || audioCtx.destination);
